...
(defun sfdx:push-file
   ...
   "Push source salesforce to org"
   (interactive)
   (let* ((file (cond (file-path file-path)
                      ((not file-path) (buffer-file-name))))
          (push-command (list "sfdx" "force:source:deploy" "-p" file)))
    (sfdx--helper:execute-async-command push-command (format "Deploy %s success" file))))
#+end_src
...
(defun sfdx:retrieve-file
   ...
   (let ((file (cond (file-path file-path)
                     ((not file-path) (buffer-file-name))))
         (retrieve-command (list "sfdx" "force:source:retrieve" "-p" file)))

    (sfdx--helper:execute-async-command retrieve-command (format "Retrieve %s success" file))))
#+end_src
...
**** SCRATCH/SANBOX ORG
#+begin_src emacs-lisp :tangle yes
(defun sfdx:authorize-sandbox-org ()
...
**** REFRESH ORG
#+begin_src emacs-lisp :tangle yes
(defun sfdx:refresh-default-org
    ()
...
#+begin_src emacs-lisp :tangle yes
(defun sfdx:fetch-salesforce-file
    ()
